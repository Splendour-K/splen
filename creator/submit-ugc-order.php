...
if (!$ugc_order) {
    die("UGC Order not found or not accepting submissions.");
}
...
if ($creator['verification_status'] !== 'verified') {
    $error = "You must be verified to submit UGC videos. Complete your profile verification first.";
}

if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$error) {
    if (strtotime($ugc_order['deadline']) <= time()) {
        $error = "This order is no longer accepting submissions.";
        ...
        if (!in_array($video_file['type'], $allowed_types)) {
            ...
            $upload_dir = '../uploads/ugc_submissions/';
            if (!is_dir($upload_dir)) {
                mkdir($upload_dir, 0755, true);
            }

            $file_extension = pathinfo($video_file['name'], PATHINFO_EXTENSION);
            $filename = 'ugc_' . $ugc_order_id . '_' . $creator['id'] . '_' . time() . '.' . $file_extension;
            $filepath = $upload_dir . $filename;

            if (move_uploaded_file($video_file['tmp_name'], $filepath)) {
                // Generate a real watermarked preview the brand will see until they approve.
                $wm_filename = 'wm_' . $filename;
                $wm_filepath = $upload_dir . $wm_filename;
                @set_time_limit(180); // watermarking can take a while for big files
                generate_video_watermark($filepath, $wm_filepath);

                $clean_rel = '/uploads/ugc_submissions/' . $filename;
                $wm_rel    = file_exists($wm_filepath) ? '/uploads/ugc_submissions/' . $wm_filename : $clean_rel;

                $sql = "INSERT INTO ugc_order_submissions (
                    ugc_order_id, creator_id,
                    video_file, watermarked_preview_file, clean_video_file, status
                ) VALUES (?, ?, ?, ?, ?, 'submitted')";

                $stmt = $pdo->prepare($sql);
                try {
                    $stmt->execute([
                        $ugc_order_id, $creator['id'],
                        $clean_rel,
                        $wm_rel,
                        $clean_rel
                    ]);

                    $success = "Your video has been submitted! The brand will review and approve soon.";
                    ...
                    create_notification_batch(
                        ...
                        'ugc_submission',
                        'brand/ugc-order-review.php?order_id=' . $ugc_order_id,
                        'ugc',
                        $ugc_order_id
                    );
                } catch (Exception $e) {
        ...
        <main class="flex-1 space-y-8">
            <header class="p-8 bg-white dark:bg-gray-900 rounded-[2rem] border border-gray-100 dark:border-gray-800 shadow-sm relative overflow-hidden">
                ...
                <div class="relative">
                    ...
                    <p class="text-gray-600 dark:text-gray-400 mt-2">Submit your UGC video</p>
                    <p class="text-sm text-gray-500 mt-3">
                        Deadline: <?php echo date('M d, Y', strtotime($ugc_order['deadline'])); ?>
                    </p>
                </div>
            </header>
            ...
            <?php if ($success): ?>
                <div class="p-8 bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-800 rounded-[2rem] text-center shadow-sm">
                    ...
                    <h3 class="text-2xl font-bold text-green-900 dark:text-green-400 mb-2">Video Submitted!</h3>
                    <p class="text-green-700 dark:text-green-300 mb-8"><?php echo $success; ?></p>
                    <a href="<?php echo APP_URL; ?>creator/ugc-orders.php" class="inline-flex h-12 items-center justify-center px-8 bg-secondary text-white font-bold rounded-full hover:scale-105 transition">View My Submissions</a>
                </div>
            <?php elseif ($error): ?>
            ...
            <form method="POST" enctype="multipart/form-data" class="space-y-8">
                <section class="p-8 bg-white dark:bg-gray-900 rounded-[2.5rem] border border-gray-100 dark:border-gray-800 shadow-sm space-y-6">
                    ...
                    <div class="p-4 bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700">
                        <p class="text-sm text-gray-600 dark:text-gray-400 mb-2"><strong>Payment per Video:</strong></p>
                        <p class="text-2xl font-black text-secondary"><?php echo e($ugc_order['currency']); ?> <?php echo number_format((float)$ugc_order['budget_per_creator'], 2); ?></p>
                    </div>

                    <?php if ($ugc_order['full_description'] ?? false): ?>
                        <div>
                            <p class="text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">What they're looking for:</p>
                            <p class="text-gray-600 dark:text-gray-400 text-sm"><?php echo nl2br(e($ugc_order['full_description'])); ?></p>
                        </div>
                    <?php endif; ?>

                    <?php if ($ugc_order['usage_rights_package'] ?? false): ?>
                        <div>
                            <p class="text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Usage Rights:</p>
                            <p class="text-gray-600 dark:text-gray-400 text-sm"><?php echo ucfirst(e($ugc_order['usage_rights_package'])); ?></p>
                        </div>
                    <?php endif; ?>
                </section>
                ...
                <section class="p-8 bg-white dark:bg-gray-900 rounded-[2.5rem] border border-gray-100 dark:border-gray-800 shadow-sm space-y-6">
                    ...
                    <div>
                        <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-3">Video File</label>
                        <div id="upload-zone" class="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl p-8 text-center">
                        ...
                        <div class="p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl">
                            <label class="flex items-start gap-3 cursor-pointer">
                                ...
                                <span class="text-sm text-amber-900 dark:text-amber-200 font-medium">
                                    <strong>This is my final video.</strong> I've reviewed the playback above and confirm this is the file I want to send to the brand.
                                </span>
                            </label>
                        </div>
                    ...
                    <div class="p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl">
                        <p class="text-xs text-blue-700 dark:text-blue-400"><strong>Note:</strong> Submissions must be uploaded files — no external links. The brand will see a watermarked preview first. Once approved, they unlock the clean video and your payment is released through Splennet.</p>
                    </div>
                </section>
                ...
                <div class="flex gap-4">
                    ...
                    <a href="<?php echo APP_URL; ?>creator/ugc-orders.php" class="flex-1 py-4 bg-gray-200 dark:bg-gray-800 text-gray-900 dark:text-white font-bold rounded-xl text-center hover:scale-105 transition">
                        Cancel
                    </a>
                </div>
            </form>
